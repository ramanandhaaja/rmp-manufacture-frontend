...
const MenuCollapse = (props) => {
  const { children, className, eventKey, expanded, label, onToggle, heightUl } = props;
  ...
  const getChildrenHeight = () => {
    ...
    if (isExpanded && children && children.length) {
      let authorityView = children?.filter((task) =>
        !task?.props?.authority?.length ? true :
          task?.props?.authority?.some((role) => userAuthority?.includes(role))
      )
      height = authorityView.length * menuItemHeight;
    }
    if (isExpanded && children && !children.length) {
  ...
  return (
    <div className="menu-collapse side-nav-custom">
      ...
      <CollapseContextProvider value={isExpanded}>
        <motion.ul
          className={direction === "rtl" ? "mr-5" : `ml-5 ${heightUl}`}
